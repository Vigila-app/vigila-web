...
import { ServiceCatalogTypeEnum, ServiceCatalogTypeLabels } from "@/src/enums/services.enums";
...
export function buildMatchingRequestFromAnswers(answers: Record<string, any>) {
  ...
  if (availabilityRules.length > 0) {
    selectedDays = Array.from(new Set(availabilityRules.map((r: any) => Number(r.weekday))));
  } else if (raw.services && typeof raw.services === "object") {
    selectedDays = Array.from(new Set(Object.keys(raw.services).map((k) => Number(k))));
  ...
  const startRaw = raw["start-date"] || raw.startDate || (availabilityRules[0] && availabilityRules[0].valid_from);
  ...
  const labelToEnum = Object.keys(ServiceCatalogTypeLabels).reduce((acc: Record<string, string>, key) => {
  ...
  selectedDays.forEach((day) => {
    ...
    const rule = availabilityRules.find((r: any) => Number(r.weekday) === Number(day));
    ...
    try {
      ...
      if (Array.isArray(svcArr) && svcArr.length > 0) serviceName = String(svcArr[0]);
    } catch (e) {
      /* ignore */
    }
  ...
  if (raw?.address) {
    if (typeof raw.address === "object" && raw.address.address && typeof raw.address.address === "object") {
  ...
  } else if (raw?.addresses && typeof raw.addresses === "object" && raw.addresses.address) {
...
export const MatchingService = {
  match: (userId: string, payload?: any): Promise<any> =>
    new Promise(async (res, rej) => {
      try {
        ...
        const response = await ApiService.post(apiMatching.MATCH(), requestBody);
        res(response);
      } catch (error) {
