...
const CheckoutFormComponent = ({
  ...
  const handleSubmit = async (event?: React.FormEvent) => {
    try {
      ...
      console.log("Confirming payment", elements);
      ...
      if (result.error) {
        ...
        // If there's no error, prefer to ensure we actually have a PaymentIntent
        showToast({
          message: "Pagamento completato con successo!",
          type: ToastStatusEnum.SUCCESS,
        });

        // result.paymentIntent may be undefined in some flows (redirects),
        // so handle that case gracefully and log the full result for debugging.
        if (result.paymentIntent && result.paymentIntent.id) {
          onSuccess(result.paymentIntent.id);
        } else {
          console.warn("confirmPayment returned no PaymentIntent:", result);
          const fallbackMsg =
            "Pagamento avviato. Verifica lo stato nella pagina di conferma.";
