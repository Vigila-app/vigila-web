...
export const Services = ({
  ...
  // Reset to first day when the set of selected days changes (adjust-during-render pattern)
  const [prevDaysLength, setPrevDaysLength] = useState(selectedDays.length);
  if (prevDaysLength !== selectedDays.length) {
    setPrevDaysLength(selectedDays.length);
    setCurrentDayIdx(0);
  }
  ...
  // Load per-day values when current day changes (adjust-during-render pattern)
  const dayKey = `${currentDayIdx}-${selectedDays.join("-")}`;
  ...
  if (lastDayKey !== dayKey) {
    ...
    if (saved) {
      ...
      setActiveService(
        loadedServices.includes(activeService || "")
          ? activeService
