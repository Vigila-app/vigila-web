...
const unauthorizedError = () =>
  NextResponse.json(
    {
      code: ResponseCodesConstants.USER_TERMS_UNAUTHORIZED.code,
      success: false,
    },
    { status: 401 }
  );
...
const requestHandler = async (
  ...
  console.log(`API ${method} user/${uid}/terms`);
  ...
  switch (method) {
    case "GET": {
      try {
        ...
        const response = await firestore.doc(`users-terms/${uid}`).get();
        if (response) {
          return NextResponse.json(
            {
    ...
    case "PUT": {
      try {
        ...
        await initAdmin();
        const firestore = getFirestore();
        const docRef = firestore.doc(`users-terms/${uid}`);
    ...
    default: {
      return NextResponse.json(
        {
          code: ResponseCodesConstants.USER_TERMS_METHOD_NOT_ALLOWED.code,
          success: false,
        },
        { status: 405 }
      );
